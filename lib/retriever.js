...
// zapíná debug výpis kontextu (top chunky) při dotazu: WENKU_DEBUG_CTX=1
...
async function retrieveChunksForDoc(sessionId, query, opts = {}) {
  ...
  // Debug výpis toho, co vlastně posíláme modelu (top chunky)
  if (DEBUG_CTX) {
    const top = ranked.slice(0, maxChunks).map(c => ({
      page: Number.isFinite(c.pageStart)
        ? c.pageStart
        : Number.isFinite(c.page)
        ? c.page
        : 0,
      score: typeof c.score === "number" ? c.score : 0,
      preview: (c.text || "").slice(0, 260).replace(/\s+/g, " ")
    }));
    console.log("[ASK ctx]", {
      sessionId,
      query,
