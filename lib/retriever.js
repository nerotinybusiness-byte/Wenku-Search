...
export function rankBM25(query, chunks) {
  ...
  const idf = {};
  for (const t of terms) idf[t] = Math.log(1 + (N - (df[t] || 0) + 0.5) / ((df[t] || 0) + 0.5));
...
export function pickExcerpts(query, ranked, pages) {
  const results = [];
  for (const c of ranked.slice(0, 3)) {
    // najdi krátký výňatek z originální "page"
