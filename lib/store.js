...
function listSessions() {
  const g = _getGlobal();
  return Array.from(g.sessions.values());
}

function findSessionByFileHash(fileHash) {
  if (!fileHash) return null;
  return listSessions().find(s => s.fileHash === fileHash) || null;
}

module.exports = { ensureSession, putSession, getSession, listSessions, findSessionByFileHash };
